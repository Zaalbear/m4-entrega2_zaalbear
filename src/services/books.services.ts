...
export class BooksServices{
    ...
    postBooks(name: string, pages:number, category: string){
        const newBook: IBooks = {id: generateId(), name, pages, category, createdAt: this.date, updatedAt: this.date } 
    ...
    updateBooks(id: string, name?: string, pages?: number, category?: string): IBooks {
    ...
    deleteBook(id: string): any {
        const deleteBook = booksDatabase.findIndex((product) => {product.id === Number(id)})
